...
-- statement-breakpoint
CREATE INDEX IF NOT EXISTS routes_enabled_slug_idx ON routes (slug) WHERE enabled = TRUE AND deleted_at IS NULL;
-- statement-breakpoint
...
-- statement-breakpoint
CREATE INDEX IF NOT EXISTS cyber_rules_active_priority_idx ON cyber_rules (priority DESC, id ASC) WHERE enabled = TRUE;
-- statement-breakpoint
...
-- statement-breakpoint
CREATE INDEX IF NOT EXISTS request_traces_user_created_idx ON request_traces (external_user_id, created_at DESC) WHERE external_user_id <> '';
-- statement-breakpoint
CREATE INDEX IF NOT EXISTS request_traces_decision_created_idx ON request_traces (decision, created_at DESC);
-- statement-breakpoint
CREATE INDEX IF NOT EXISTS request_traces_risk_created_idx ON request_traces (risk_code, created_at DESC) WHERE risk_code <> '';
-- statement-breakpoint
...
-- statement-breakpoint
CREATE INDEX IF NOT EXISTS outbox_events_pending_idx ON outbox_events (next_attempt_at, id) WHERE published_at IS NULL;
-- statement-breakpoint
...
-- statement-breakpoint
INSERT INTO cyber_rules (code, name, description, category, pattern, pattern_type, action, priority, enabled) VALUES
(
