...
func (s *EventSink) Wait() { s.wg.Wait() }

func (s *EventSink) Enabled() bool { return s.enabled }

func (s *EventSink) QueueDepth() int { return len(s.queue) }
...
func (s *EventSink) writeWorker(ctx context.Context) {
	...
	flush := func(flushCtx context.Context) {
		...
		if err := s.writer.WriteMessages(flushCtx, messages...); err != nil {
			s.log.Error("Kafka batch publish failed; moving events to PostgreSQL outbox", "count", len(pending), "error", err)
			for _, event := range pending {
...
func (s *EventSink) enqueueOutbox(event queuedKafkaEvent, publishErr error) {
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Second)
	defer cancel()
	if err := s.store.EnqueueOutbox(ctx, s.topic, event.key, event.payload, publishErr.Error()); err != nil {
	...
	for {
		...
		case <-ticker.C:
			...
			for _, event := range events {
				message := kafka.Message{Topic: event.Topic, Key: []byte(event.Key), Value: event.Payload, Time: time.Now().UTC()}
				writeCtx, cancel := context.WithTimeout(ctx, 10*time.Second)
				err := s.writer.WriteMessages(writeCtx, message)
				cancel()
				if err != nil {
...
// ApplyRetention updates retention.ms for the configured event topic. The Kafka principal
// must have ALTER_CONFIGS permission. The requested value is also stored by the admin API.
		...
		Resources: []kafka.IncrementalAlterConfigsRequestResource{
			{
				...
				Configs: []kafka.IncrementalAlterConfigsRequestConfig{
					{Name: "retention.ms", Value: strconv.FormatInt(int64(days)*24*60*60*1000, 10), Operation: 0},
				},
			},
		},
	})
