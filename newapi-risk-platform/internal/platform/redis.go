...
func NewRedisGuard(ctx context.Context, cfg Config, log *slog.Logger) *RedisGuard {
	...
	if cfg.RedisURL == "" {
		log.Warn("Redis is disabled; distributed rate limits use process-local fallback")
		return guard
	}
	...
	defer cancel()
	if err := guard.client.Ping(pingCtx).Err(); err != nil {
...
func (g *RedisGuard) Available() bool { return g.available.Load() }
...
func (g *RedisGuard) Allow(ctx context.Context, key string, rate float64, burst int) bool {
	...
	if g.client != nil {
		ttlMS := int64(float64(burst)/rate*2000) + 1000
		result, err := g.client.Eval(ctx, tokenBucketScript, []string{"risk:rate:" + key},
			time.Now().UnixMilli(), rate, burst, ttlMS).Int64()
		if err == nil {
...
func (g *RedisGuard) Acquire(ctx context.Context, key string, limit int, ttl time.Duration) (func(), bool) {
	...
	if g.client != nil {
		...
		result, err := g.client.Eval(ctx, acquireSemaphoreScript, []string{"risk:sem:" + key},
			now, now+ttl.Milliseconds(), limit, token).Int64()
		if err == nil {
			g.available.Store(true)
			if result != 1 {
				return nil, false
			...
			return func() {
				...
				defer cancel()
				_ = g.client.ZRem(releaseCtx, "risk:sem:"+key, token).Err()
			}, true
		}
		g.available.Store(false)
...
func (g *RedisGuard) UseNonce(ctx context.Context, keyID, nonce string, ttl time.Duration, store *Store) (bool, error) {
	if g.client != nil {
...
func (g *RedisGuard) PushDLQ(ctx context.Context, stream string, fields map[string]any) error {
	...
	if err := g.client.XAdd(ctx, args).Err(); err != nil {
		g.available.Store(false)
		return err
	}
	g.available.Store(true)
