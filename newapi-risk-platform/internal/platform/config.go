...
func (c Config) Validate() error {
	...
	if c.BootstrapAdminUsername == "" {
		problems = append(problems, "BOOTSTRAP_ADMIN_USERNAME is required")
	}
	if len(c.BootstrapAdminPassword) < 14 {
		problems = append(problems, "BOOTSTRAP_ADMIN_PASSWORD must contain at least 14 characters")
	}
	...
	if c.PostgresMaxConns < 2 || c.PostgresMaxConns > 1000 {
		problems = append(problems, "POSTGRES_MAX_CONNS must be between 2 and 1000")
	}
	if c.PostgresMinConns < 0 || c.PostgresMinConns > c.PostgresMaxConns {
		problems = append(problems, "POSTGRES_MIN_CONNS must be non-negative and no greater than POSTGRES_MAX_CONNS")
	}
	...
	if c.TraceQueueSize < 100 || c.TraceQueueSize > 1000000 {
		problems = append(problems, "TRACE_QUEUE_SIZE must be between 100 and 1000000")
	}
	if c.TraceBatchSize < 1 || c.TraceBatchSize > 5000 {
		problems = append(problems, "TRACE_BATCH_SIZE must be between 1 and 5000")
	}
	...
	if c.KafkaEnabled {
		if len(c.KafkaBrokers) == 0 {
			problems = append(problems, "KAFKA_BROKERS is required when KAFKA_ENABLED=true")
		}
		if c.KafkaTopic == "" {
			problems = append(problems, "KAFKA_TOPIC is required when KAFKA_ENABLED=true")
		}
		if c.KafkaSASLMechanism != "" && c.KafkaSASLMechanism != "plain" && c.KafkaSASLMechanism != "scram-sha-256" && c.KafkaSASLMechanism != "scram-sha-512" {
			problems = append(problems, "KAFKA_SASL_MECHANISM must be plain, scram-sha-256, scram-sha-512, or empty")
		}
		if c.KafkaSASLMechanism != "" && (c.KafkaUsername == "" || c.KafkaPassword == "") {
			problems = append(problems, "KAFKA_USERNAME and KAFKA_PASSWORD are required when SASL is enabled")
		}
	}
	if strings.EqualFold(c.Environment, "production") && c.AllowPrivateUpstreams {
		// Explicitly allowed, but make the operator acknowledge it through a second flag.
		if !envBool("ACK_PRIVATE_UPSTREAM_SSRF_RISK", false) {
			problems = append(problems, "ACK_PRIVATE_UPSTREAM_SSRF_RISK=true is required with ALLOW_PRIVATE_UPSTREAMS=true in production")
		}
	}
	if len(problems) > 0 {
